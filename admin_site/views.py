...
def all_table_data():
    ...
    all_data = {
        ...
        "total_no_auth_app_datas": total_no_auth_app_datas,
...
def index(request):
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "admin_site/index.html",
            {
                "total_admins": all_table_data()["admins"],
...
def app_user(request):
    ...
    if request.user.is_authenticated:
        ...
        if "datefilter" in request.GET:
            ...
            if val == "ninety":
                locals = locals.filter(
                    date_joined__gte=datetime.now() - timedelta(days=90),
                    date_joined__lte=datetime.now(),
                )
...
def profile_model(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "admin_site/profile_model.html",
            {
                "total_profiles": page_obj,
                "total_records": len(page_obj),
                "country_list": country_list,
                "city_list": city_list,
                "first_record": first_record,
                "last_record": last_record,
                "smallest_age_record": smallest_age_record,
                "biggest_age_record": biggest_age_record,
                "filter_mobile_val": filter_mobile_val,
                "searched_lat": latitude,
                "searched_long": longitude,
            },
        )
    else:
        return redirect("login")
...
def user_preference_model(request):
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "admin_site/user_preferences_model.html",
            {"total_details": page_obj},
        )
    else:
        return redirect("login")
...
def no_auth_app_data_model(request):
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "admin_site/app_data_no_auth_model.html",
            {"total_no_auth_app_datas": page_obj},
        )
    else:
        return redirect("login")
...
def purchase_model(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(
            request, "admin_site/purchase_model.html", {"total_purchases": page_obj}
        )
    else:
        return redirect("login")
...
def tag_model(request):
    if request.user.is_authenticated:
        ...
        if "search" in request.GET:
            searchvalue = request.GET["search"]
            total_tags = Tag.objects.filter(Q(tag__icontains=searchvalue))
...
def product_model(request):
    if request.user.is_authenticated:
        ...
        return render(
            request, "admin_site/product_model.html", {"total_products": page_obj}
        )
    else:
        return redirect("login")
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            url = (
                ...
                + ","
                + long
                + ".json?types=poi&access_token=pk.eyJ1IjoiYXJhYmFwcCIsImEiOiJjbDh2YmtiODQwNXo4M29udTA0eWxldmIxIn0.tzc8bwS-5vvdE32_T0EY7A"
            )
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if user_stared_Textart != "[]":
                obj.user_stared_Textart = user_stared_Textart.replace("[", "{").replace(
                    "]", "}"
                )
            if user_stared_colors != "[]":
                obj.user_stared_colors = user_stared_colors.replace("[", "{").replace(
                    "]", "}"
                )
            if user_stared_fonts != "[]":
                obj.user_stared_fonts = user_stared_fonts.replace("[", "{").replace(
                    "]", "}"
                )
            if most_used_fonts != "[]":
                obj.most_used_fonts = most_used_fonts.replace("[", "{").replace(
                    "]", "}"
                )
            if user_custom_colors != "[]":
                obj.user_custom_colors = user_custom_colors.replace("[", "{").replace(
                    "]", "}"
                )
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            try:
                ...
                messages.error(
                    request,
                    "Start date in incorrect date format. It should be YYYY-MM-DD.",
                )
                return HttpResponseRedirect(request.META.get("HTTP_REFERER"))
            ...
            try:
                ...
                messages.error(
                    request,
                    "End date in incorrect date format. It should be YYYY-MM-DD.",
                )
                return HttpResponseRedirect(request.META.get("HTTP_REFERER"))
            ...
            if request.user.check_password(old_password):
                if new_password == confirm_password:
                    ...
                    if mat:
                        ...
                        messages.error(
                            request,
                            "password must be include atleast one special character,number,small and capital letter and length between 6 to 20!!!",
                        )
                else:
                    messages.error(
        ...
        try:
            ...
            if mat:
                if new_password == confirm_password:
                    ...
                    messages.error(
                        request, "new password and confirm password doesnot matched!!!"
                    )
            else:
                messages.error(
                    request,
...
def filter(request):
    if request.user.is_authenticated:
        ...
        if "fromtodate" in request.GET:
            ...
            end_date = datetime.strptime(end_date + " 23:59:59", format_data)
            total_profiles = Profile.objects.filter(
                created_at__startswith=start_date, created_at__endswith=end_date
            )
